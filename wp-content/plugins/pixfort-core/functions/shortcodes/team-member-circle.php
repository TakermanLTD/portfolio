		...
		$output .= '<div class="'. esc_attr( $css_class ) .' '.$position.' '.$anim_class.'" style="position:relative" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';
            ...
            $output .= '</div>';
                $output .= '<div class="'.$position.'">';
                    $output .= '<'.$name_tag.' class="card-title mb-3 '.$name_classes.' '.$anim_class.'" '.$name_style.' data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">'.$name.'</'.$name_tag.'>';
					$delay+=100;
                    $output .= '<'.$title_tag.' class="mb-3 '.$title_classes.' '.$anim_class.'" '.$title_style.' data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'"><strong>'.$title.'</strong></'.$title_tag.'>';
					$delay+=100;
                    if(!empty($description)) $output .= '<p class="p-0 '.$anim_class.' '.$d_classes.'" '.$d_custom_color.' data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">'. do_shortcode( $description ) .'</p>';
