 ...
 if( ! function_exists( 'sc_pix_photo_box' ) )
 {
    function sc_pix_photo_box( $attr, $content = null )
    {
		...
		$output .= '<div class="card w-100 h-100 bg-heading-default bg-transparent '.$class_names.' pix-hover-item rounded-lg position-relative overflow-hidden text-white '.$class_names.'" '.$anim_type.' '.$anim_delay.' '.$jarallax.'>
		  <img src="'.PIX_IMG_PLACEHOLDER .'" data-srcset="'.$imgSrcset.'" data-src="'.$imgSrc.'" class="pix-lazy card-img pix-bg-image h-100 pix-img-scale pix-opacity-10 '.$color_effect.'" loading="lazy" alt="...">
		  <a href="'.$link.'" '.$target_out.' class="card-img-overlay2 d-inline-block w-100 pix-img-overlay pix-p-10 d-flex align-items-end" '.$box_height.'>
		  	<div class="w-100">';
		    if(!empty($title)){
